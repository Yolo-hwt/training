    ...
    <div class="panels-container">
      <div class="panel left-panel">
        <div class="content">
          <h3>初次使用 ?</h3>
          <p>
            新用户? 注册港口库场流动机械运行监控系统
            以提供更好安全保护与使用体验 点击按钮前往注册
          </p>
...
export default {
  ...
  methods: {
    ...
    //登录验证
    async verifyUserIdAndPsd(userId, userPsd) {
      let flag = false;
      //root测试用户登录
      if (userId === "root" && userPsd === "123") {
        flag = true;
        return flag;
      }
      //调用登录接口
    ...
    async handleSignInSubmit() {
      //空值判断
      const nullflag_name = this.signInUserId === "" ? true : false;
      const nullflag_psd = this.signInUserPsd === "" ? true : false;
      // const psd = this.signInUserPsd;
      if (nullflag_name || nullflag_psd) {
        alert("账号或密码不能为空");
        // console.log("login flag:", nullflag_name, nullflag_psd);
      } else {
        //验证账号密码是否正确
        const loginResult = await this.verifyUserIdAndPsd(
          this.signInUserId,
          this.signInUserPsd
        );
